...
def data_preprocessing(training_data):
    ...
    labels = torch.tensor(df[label_names].to_numpy())

    # cased_tokenizer = SubwordTokenizer("/app/resources/bert-base-uncased-hash.txt", do_lower_case=True)
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased", do_lower_case=True)
    tokenizer_output = tokenizer(df.data.to_list(),
                                 max_length=256,
                                 padding='max_length',
                                 return_tensors='pt',
                                 truncation=True,
