...
def data_preprocessing(training_data):
    ...
    train_dataloader = DataLoader(dataset=training_dataset, sampler=train_sampler, batch_size=32)
    val_dataloader = DataLoader(dataset=validation_dataset, sampler=validation_sampler, batch_size=64)
    return train_dataloader, val_dataloader, idx2label


def train_model(model_dir, train_dataloader, idx2label, core_context, device):
...
def main(core_context):
    ...
    model, steps_completed = train_model(args.model_dir, train_dataloader, idx2label, core_context, device)
